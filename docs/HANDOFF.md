...
- 清扫“规划参数”还包含倒车速度、最大转弯角速度、线/角加速度及换向/分段附加时间；
  ...
  不是秒，不直接参与离线排序；覆盖转场执行由带档位 Hybrid A* + 在线 TEB 最终裁决，
  普通点到点仍使用 Navfn；
...
## 2026-08-30 覆盖转场 Hybrid A* 架构（本地完成，未部署）

- 修改前的完整工作树已保存到分支 `0830`，快照提交为 `7384ec7`。该提交包含当时已有的
  Qt、FOD、未知区保护、覆盖入口容差等改动；可复现下载的 354 MB CLIP 权重按仓库规则忽略。
- `CoverageGlobalPlanner` 现有三种互斥模式：普通点到点使用 Navfn；覆盖入场和条带间转场
  使用 Hybrid A*；扫掠段继续使用管理器强制的精确清扫线。覆盖模式交接过期或 Hybrid A*
  无解时 fail-closed，不会在覆盖所有权仍有效时偷偷回退 Navfn。
- Hybrid A* 在每次 move_base 全局规划周期复制最新全局 costmap，状态为
  `(x,y,yaw,前进/倒车,转向档)`；按 `1.35 m` 最小半径积分恒曲率运动原语，每 `0.10 m`
  对完整车辆 footprint 做碰撞检查，并拒绝未知、致命障碍及地图外。代价使用任务当前前进/
  倒车速度、最大角速度、换向附加时间以及转向变化/障碍余量；规划上限默认为 `1.5 s`、
  `80000` 次扩展。
- `SetEnforcedPath` 新增 `TransitProfile`，管理器在提交 action goal 前原子交接本次任务的
  ...
  直线跟踪配置保持不变，任务终态仍完整恢复普通导航基线。车辆已在入口位置容差内但航向
  超差时，不再沿用旧 Navfn 架构直接判阻塞，而是交给 Hybrid A* 搜索驶离再返回的调整路径。
...
- NVIDIA 本地全工作区 Release 构建通过；覆盖包 `14` 项契约、`21` 项几何、`78` 项状态机
  和 `4` 项 Hybrid A* GTest 全部通过，`catkin_test_results` 汇总 `121` 项、0 失败。尚未
  另外全项目 `health_check.sh --static` 汇总 `797` 项、0 失败（仅提示现场无麦克风设备）。
  尚未部署 J6M、未重启双机栈、未发送目标或非零速度；新的消息 MD5 和 ARM64 install
  仍需在正式部署时成套更新并做低速实车验收。
