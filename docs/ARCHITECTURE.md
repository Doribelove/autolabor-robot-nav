...
实时位姿到最终入口整段重算。异常 action 确认终态后，管理器采用 Qt 冻结在任务中的
`0–10 s` 重试等待参数，默认 `0.5 s`，其中 `0 s` 表示立即重试。区域不是 geofence，
转场可在完整已知自由地图中绕行；覆盖
管理器只在静态地图模式启动。

清扫中断后的进度保存在当前分段的执行上下文中，并绑定原计划与导航所有权 token；
新任务/区域会创建新的上下文，不支持节点重启后自动恢复。可信异常终态或超时取消后，
管理器在等待期间继续观察原线的连续进度及出口完成条件，确认物理停稳，再向全局插件
提交当前投影点至原终点的剩余直线。原起终点、入口历史和完成阈值不裁剪。普通阻挡
不消耗转场重试预算，也不跳过本线；人工/外部暂停和定位、感知、底盘故障仍优先阻止重发。
大于 `0.50 m` 的单次位姿跳变、缺少入口历史或恢复时超出 `0.30 m / 0.35 rad` 的
原线走廊会拒绝自动续扫。首次清扫目标提交前仍可做原有有限入口恢复，启动后不自动倒回
入口。实现和本机验证边界见 [清扫中断恢复修正](COVERAGE_SWEEP_RESUME_20260906.md)。
