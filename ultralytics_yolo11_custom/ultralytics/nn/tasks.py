...
class BaseModel(torch.nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        source_layers = getattr(model, "model", ())
        target_gam = [i for i, layer in enumerate(self.model) if isinstance(layer, GAM_Attention)]
        source_gam = [i for i, layer in enumerate(source_layers) if isinstance(layer, GAM_Attention)]
        insert_at = target_gam[0] if len(target_gam) == 1 else -1
        shifted_architecture_matches = insert_at >= 0 and all(
            type(source_layer) is type(self.model[i if i < insert_at else i + 1])
            for i, source_layer in enumerate(source_layers)
        )
        if not source_gam and len(self.model) == len(source_layers) + 1 and shifted_architecture_matches:
            # The custom YOLO11-GAM model inserts GAM into the backbone, shifting every original head layer by one.
            # Remap official YOLO11 keys so pretrained head weights are not silently discarded by exact-key matching.
            remapped_csd = {}
            for key, value in csd.items():
                match = re.match(r"model\.(\d+)(\..+)", key)
                if match and int(match.group(1)) >= insert_at:
                    key = f"model.{int(match.group(1)) + 1}{match.group(2)}"
                remapped_csd[key] = value
            csd = remapped_csd
            if verbose:
                LOGGER.info(f"Remapped pretrained layers {insert_at}+ by +1 for inserted GAM_Attention")
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c1 = ch[f]
            args = [*args[1:]]
        elif m is GAM_Attention:  # GAM preserves channels: input=output=ch[f]
            c2 = ch[f]
            args = [c2, c2, *args]  # GAM_Attention(c1, c2, group, rate, shortcut)
        else:
            c2 = ch[f]
