...
class BaseTrainer:
    ...
    def build_optimizer(self, model, name="auto", lr=0.001, momentum=0.9, decay=1e-5, iterations=1e5):
        ...
        for module_name, module in unwrap_model(model).named_modules():
            for param_name, param in module.named_parameters(recurse=False):
                ...
                if param.ndim >= 2 and use_muon:
                    ...
                    g[2][fullname] = param
                elif isinstance(module, bn) or "logit_scale" in fullname or param_name == "gamma":  # no decay
                    # Norms, contrastive logit scales, and residual attention gates should not be weight-decayed.
                    g[1][fullname] = param
        ...
        if use_muon:
            ...
            # Use the actual detection head path instead of a hard-coded layer index. Custom architectures may insert
            # modules before Detect (e.g. GAM), which shifts the official YOLO head from model.23 to model.24.
            classification_heads = tuple(
                f"{module_name}.cv3"
                for module_name, module in unwrap_model(model).named_modules()
                if hasattr(module, "cv3") and any(cls.__name__ == "Detect" for cls in type(module).mro())
            )

            def use_higher_lr(param_name):
                """Return whether MuSGD should use the fine-tuning classification-head learning rate."""
                return (
                    any(param_name == prefix or param_name.startswith(f"{prefix}.") for prefix in classification_heads)
                    or "proto.semseg" in param_name
                    or "flow_model" in param_name
                )

            g_ = []  # new param groups
            for x in g:
                p = x.pop("params")
                p1 = [v for k, v in p.items() if use_higher_lr(k)]
                p2 = [v for k, v in p.items() if not use_higher_lr(k)]
