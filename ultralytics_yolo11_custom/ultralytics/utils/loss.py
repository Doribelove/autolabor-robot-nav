...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        class_pos_weights = getattr(model, "class_pos_weights", None)
        if class_pos_weights is not None:
            class_pos_weights = class_pos_weights.to(device=device, dtype=torch.float)
            if class_pos_weights.numel() != m.nc:
                raise ValueError(
                    f"Expected {m.nc} class positive weights, received {class_pos_weights.numel()}"
                )
        self.bce = nn.BCEWithLogitsLoss(reduction="none", pos_weight=class_pos_weights)
        self.hyp = h
