...
def img2label_paths(img_paths: list[str]) -> list[str]:
    """Convert image paths to label paths for standard and project-specific directory layouts."""
    mappings = (
        (f"{os.sep}images_pro{os.sep}", f"{os.sep}labels_pro{os.sep}"),
        (f"{os.sep}images{os.sep}", f"{os.sep}labels{os.sep}"),
    )
    label_paths = []
    for image_path in img_paths:
        label_path = image_path
        for image_segment, label_segment in mappings:
            if image_segment in image_path:
                label_path = label_segment.join(image_path.rsplit(image_segment, 1))
                break
        label_paths.append(label_path.rsplit(".", 1)[0] + ".txt")
    return label_paths
