...
def main():
    ...
    send_json(writer, {"ready": True, "demo_only": True, "continuous": args.continuous,
                       "pid": os.getpid(), "numpy": np.__version__})
    previous = 0
    started_session = time.monotonic()
    seconds = None if args.continuous else 660
    with BoundedLog(directory / "inference.log") as log:
        log.write("WORKER_START pid=%d continuous=%s" % (os.getpid(), args.continuous))
