...
class CoverageGeometryTest(unittest.TestCase):
    ...
    def test_adjacent_boustrophedon_never_skips_rows_and_alternates_yaw(self):
        parameters = CoverageTimeParameters(
            max_forward_speed_mps=1.30,
            max_reverse_speed_mps=0.80,
            max_angular_speed_rps=1.00,
            linear_accel_mps2=2.00,
            angular_accel_rps2=1.00,
            direction_change_penalty_sec=0.50,
        )
        swaths = [
            Swath(Point(0.0, y), Point(8.0, y), y, 8.0)
            for y in (2.55, 0.0, 1.70, 0.85)
        ]
        route, estimate = order_adjacent_boustrophedon(
            swaths,
            Point(0.0, -1.0),
            1.35,
            0.0,
        ...
        scan_values = [swath.scan_v for swath in route]
        deltas = [
            second - first
            for first, second in zip(scan_values, scan_values[1:])
        ]
        self.assertTrue(
            all(abs(abs(delta) - 0.85) <= 1.0e-9 for delta in deltas)
        )
        yaws = [
            math.atan2(
    ...
    def test_region_entry_uses_reverse_when_it_reduces_total_time(self):
        ...
        # Navfn still supplies the geometric first-entry route, while its TEB
        # execution and the route time objective share the reverse setting.
        ...
        reverse_time, used_reverse = estimate_transition_time(
            Point(0.0, 0.0), 0.0, Point(-4.0, 0.0), 0.0, 1.35,
            CoverageTimeParameters(allow_reverse=True, **common),
        )
        forward_time, _ = estimate_transition_time(
            Point(0.0, 0.0), 0.0, Point(-4.0, 0.0), 0.0, 1.35,
            CoverageTimeParameters(allow_reverse=False, **common),
        )
        self.assertTrue(used_reverse)
        self.assertLess(reverse_time, forward_time)
