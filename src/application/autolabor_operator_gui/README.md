...
- 清扫页默认使用 `1.00 m` 有效宽度和 `15%` 重叠（车道中心距 `0.85 m`），显示
  ...
  推断实体清扫机构已经工作。
- 清扫页“规划参数”的全部 11 项值每次修改都立即写入当前用户的 `QSettings`，并在
  400 ms 防抖后通过 `/coverage/set_planning_defaults` 事务性更新 J6M TEB、覆盖管理器和
  当前 release 的 `coverage.yaml`。J6M 未确认前禁止确认区域或启动清扫；重连后 Qt 会把
  QSettings 再次下发。默认线加速度为 `1.00 m/s²`、重规划间隔为 `1.0 s`。速度、倒车
  许可、角速度和加速度供普通点到点、首线/跨区入场和覆盖任务共用。
  “恢复默认参数”从 J6M 的只读 `coverage_factory_defaults.yaml` 恢复整组出厂值，只有服务
  成功后才更新 Qt 控件和 QSettings。
  界面会用新鲜的 NVIDIA 看门狗状态核对实时线速度与角速度硬上限，超限时
  禁止确认区域、单区启动和队列启动，并显示明确原因。
- 执行架构显式区分：普通导航、每个区域首条清扫线和跨区域后的新首线使用一个 Navfn +
  TEB action；只有同一区域线间转场从实时位姿直接 Hybrid A* 到下一入口，并按 cusp 拆为
  固定档位 action 后交给 TEB 闭环跟踪；清扫直线使用强制路径 + TEB。安全 mux 不计算
  跟踪速度，只有在许可、指令新鲜度、档位与曲率均有效时才转发 TEB 输出。管理器只有在
  M2 `/odom` 确认零速后才提交下一档位。若路径失效，
  后端精确取消当前 generation 并从实时位姿重算。扫掠直线仍禁止倒车。
- 两个地图页都显示路线图例：青色覆盖条带、橙色完整 Hybrid A* 转场、蓝色当前全局
  参考路线、红色当前 TEB 局部轨迹、绿色覆盖执行记录。Hybrid 阶段蓝/红线只作诊断，
  橙线来自锁存的
  `/coverage/hybrid_transition_path`，可直接检查倒车段、换档点及最终清扫线入口；临时路线
  只在活动目标期间启用，任务结束后清空缓存，不显示可能穿过障碍物的直线连接预览。
...
- ZED/视觉模型画面、检测结果、曝光/增益和图像质量控制完整保留。“视觉”页只将
...
- “视觉”页右侧可选择 LocateAnything-3B（单类 `trash`）、YOLO11-GAM
  ...
  均为空、相关状态新鲜且里程计零速时启用；独立后台脚本会复核同一安全条件，原子更新
  NVIDIA/J6M 模型契约并完整冷重启。地图模式会保留，一次性的视觉运动授权不会继承。
- 视觉行驶只调用 `/fod_navigation_mode/set_fod_enabled`；局部路线暂停、停车确认
  和恢复都由安全仲裁器完成。右侧检测置信度通过统一的
  `/fod_detector/set_detection_confidence` 服务直接修改当前检测模型的真实 `conf`，
  范围为 `0.05–0.95`；后端必须回读相同值才显示成功。该值同时写入
  `/fod/vision/detector_confidence`，同一 ROS master 内切换 YOLO11-GAM 与
  `detect_and_classify` 时继承。LocateAnything 没有可校准的逐框分数，界面明确显示
  `N/A` 并禁止伪造生效。视觉行驶活动期间不允许修改。
