...
米制距离。目标从相机中部移动到画面下方并按连续帧确认消失后，程序立即以该
...
- `/m2_driver/steer_center_bias`、`/m2_driver/reset_odom`、
  ...
  `PRECHECK` 和运行期图审计要求这些话题没有发布者。转向中心标定必须在进入
  本模式前用 `rostopic pub -1` 完成，并等待该一次性发布进程退出；不得在视觉
  回收过程中修改转向中心或复位里程计。节点还会订阅这四个话题；使能后即使
  一次性发布者在两次图检查之间迅速退出，任何消息也会锁存 `ABORT`。
...
- 默认速度上限 `0.15 m/s`，画面底部降至 `0.06 m/s`，盲走为 `0.08 m/s`；
  软件前轮转角不超过 `12°`。
...
将且仅将一个无害、柔软的试验 FOD 放在画面中部，保证其初始底部锚点不在
画面最下沿，并清空车辆前方至少 `7 m`。操作员拿好物理急停后执行：
...
6 帧后才开始低速前进。目标到达画面底部中央并经连续帧确认消失后，程序以
