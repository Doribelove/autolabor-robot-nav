#!/usr/bin/env python3
"""V2-04G Gazebo-only typed Anchor transaction with rule mechanisms."""
...
class SimulationTypedAnchorTransactionNode:
    def __init__(self):
        ...
        self.mechanism = (
            RuleMechanismController(load_mechanism_config(mechanism_path))
        ...
        with self.lock:
            ...
            if context is None:
                ...
                if self.mechanism is not None:
                    geometry_message = self.geometry
                    mechanism_valid = bool(
                        geometry_message is not None
                        and geometry_message.valid and not geometry_message.stale
                        and geometry_message.world_model_seq == context.world_model_seq
                    )
                ...
                if mechanism_command is not None:
                    self.mechanism_publisher.publish(String(data=json.dumps({
                        "world_model_seq": int(context.world_model_seq),
                        "mode_seq": int(context.mode_seq),
                        "geometry_mode": geometry,
                        "dynamic_overlay": overlay,
                        "topology_preference": mechanism_command.topology_preference,
                        "topology_locked": mechanism_command.topology_locked,
                        "topology_switch_count": self.mechanism.topology_switch_count,
                        "corridor_centerline_active": (
                            mechanism_command.corridor_centerline_active
                        ),
                        "maneuver_reverse": mechanism_command.maneuver_reverse,
                        "residuals": mechanism_command.residuals,
                        "reason": mechanism_command.reason,
                    }, sort_keys=True)))
