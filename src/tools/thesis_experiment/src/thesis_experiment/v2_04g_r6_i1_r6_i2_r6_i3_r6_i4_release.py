"""R6-I4 repaired offline validation of the frozen failed R6-I3 release.

This versioned module is deliberately ROS-free and process-free.  It preserves
the R6-I3 release semantics while repairing exactly one preflight-integrity
defect: every authorization resource is still read once and rehashed, but only
the two documents whose contents are semantically consumed are parsed.  It
does not authorize the failed release, create an execution directory, create a
journal, write a log, or start a subprocess.
...
AUTHORIZATION_CLOSURE_LABEL = "inherited_r6_i2_dependency_closure"
...
class R6I3ExecutionReleaseError(ValueError):
...
class _StrictYamlLoader(yaml.SafeLoader):
    ...
    Plain aliases are accepted because the frozen I2 closure emitted by
    ...
    def flatten_mapping(self, node):
        for key_node, _ in node.value:
            if key_node.tag == "tag:yaml.org,2002:merge":
                raise R6I3ExecutionReleaseError(
...
def _construct_unique_mapping(loader, node, deep=False):
    ...
    for key_node, value_node in node.value:
        if key_node.tag == "tag:yaml.org,2002:merge":
            raise R6I3ExecutionReleaseError("YAML merge keys are forbidden")
...
def _require(condition, message):
    if not condition:
        raise R6I3ExecutionReleaseError(message)
...
def _parse_yaml_mapping(payload, label):
    ...
    except R6I3ExecutionReleaseError:
        raise
...
def _validate_schedule(schedule, label):
    ...
    return canonical_document_sha256(schedule)
    ...
    expected.update(
        {
            ...
            "release_source": "explicit_user_instruction_after_execution_readiness_closure",
...
def load_and_validate_execution_release(
    ...
    *,
    ...
    _require(
        REQUIRED_RESOURCE_LABELS.issubset(set(expected_paths)),
        "trusted release resource labels are incomplete",
    )
    _require(
    ...
    for label, declared_path in expected_paths.items():
        ...
        if Path(declared_path).suffix.lower() in {".yaml", ".yml"}:
