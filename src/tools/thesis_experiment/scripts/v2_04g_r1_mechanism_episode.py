#!/usr/bin/env python3
"""Run one preregistered V2-04G-R1 calibration-only mechanism episode."""
...
class SupervisorRepairEpisode(_LEGACY.ValidationEpisode):
    ...
    def __init__(self, instance, method, output_dir, stage, split, profile_id):
        ...
        self.mechanism_join_valid_count = 0
        self.mechanism_join_reason_counts = {}
    ...
    def _mechanism_state(self, message):
        ...
        with self.lock:
            ...
            self.mechanism_join_valid_count += int(bool(value.get("join_valid")))
            join_reason = str(value.get("join_reason", "MISSING_JOIN_REASON"))
            self.mechanism_join_reason_counts[join_reason] = (
                self.mechanism_join_reason_counts.get(join_reason, 0) + 1
            )
    ...
    def _sample(self, goal_reached=False):
        ...
        if self.stage != "V2-04G-R1" or row is None:
            return row
        with self.lock:
            scan = self.scan
    ...
    def run(self):
        ...
        if self.stage == "V2-04G-R1":
            ...
            evaluation.update({
                ...
                "mechanism_join_valid_count": self.mechanism_join_valid_count,
                "mechanism_join_valid_fraction": (
                    float(self.mechanism_join_valid_count) / self.mechanism_message_count
                    if self.mechanism_message_count else None
                ),
                "mechanism_join_reason_counts": dict(
                    self.mechanism_join_reason_counts
                ),
