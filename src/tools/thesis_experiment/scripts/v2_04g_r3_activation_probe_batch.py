#!/usr/bin/env python3
"""Run the preregistered V2-04G-R3 frozen-taxonomy readiness probes."""
...
_SPEC = importlib.util.spec_from_file_location("v2_04g_r2_frozen_helpers_r3", R2_BATCH)
...
def _summary(preregistration_path, schedule, reports, status, failure):
    all_pass = (
        status == "complete" and len(reports) == len(schedule)
        and all(report.get("all_hard_gates_pass") is True for report in reports)
    )
    taxonomy, reasons = {}, {}
    for report in reports:
        for key, value in report.get("fault_taxonomy_counts", {}).items():
            taxonomy[key] = taxonomy.get(key, 0) + value
    ...
    return {
        ...
        "planned_probe_count": len(schedule), "executed_probe_count": len(reports),
        "valid_probe_count": sum(
            report.get("all_hard_gates_pass") is True for report in reports
        ),
        "all_probe_hard_gates_pass": all_pass,
        "aggregate_fault_taxonomy_counts": taxonomy,
        "aggregate_fault_reason_counts": reasons,
        "reports": [{
            ...
            "transaction_valid_fraction": report.get("transaction_valid_fraction", 0.0),
        ...
        "failure": failure, "readiness_pass": all_pass,
        "ttc_probe_authorized": all_pass, "navigation_authorized": all_pass,
        "sac_training_authorized": False, "real_vehicle_authorized": False,
    }
...
def main():
    ...
    parser.add_argument("--listener", type=Path, default=Path(__file__).with_name(
        "v2_04g_r3_activation_probe_listener.py"))
    ...
    if not (
        ...
        raise ValueError("R3 readiness boundary drifted")
    ...
    if (args.compiled_scenes_dir / "compiled_scene_index.yaml").resolve() != expected_index.resolve():
        raise ValueError("R3 compiled scene path drifted")
    output_root = args.output_root.resolve()
    output_root.relative_to((WORKSPACE /
        "artifacts/v2/calibration/v2_04g_r3/activation_probe").resolve())
    ...
    if len(schedule) != prereg["budget"]["activation_readiness_probe_count"]:
        raise ValueError("R3 readiness budget drifted")
    if {row["seed"] for row in schedule} != set(
        prereg["seed_firewall"]["readiness_probe_only_seeds"]
    ):
        raise ValueError("R3 readiness seed schedule drifted")
    reports = []
    ...
    for row in schedule:
        ...
        if profile not in probe["profile_ids"]:
            raise ValueError("R3 readiness profile schedule drifted")
        ...
        if report_path.is_file():
            ...
            if not all(report.get(key) == value for key, value in identity.items()):
                raise RuntimeError("existing R3 readiness identity drifted")
            if report.get("all_hard_gates_pass") is not True:
                raise RuntimeError("existing R3 readiness probe is failed evidence")
        ...
        if not report_path.is_file():
            ...
            _write_yaml(summary_path, _summary(
        ...
        if (result is None or result.returncode != 0
            ...
            failure = {**row, "reason": "readiness taxonomy hard gate failed"}
            _write_yaml(summary_path, _summary(
                args.preregistration, schedule, reports, "failed", failure))
            raise RuntimeError(failure["reason"])
        print("PASS R3 readiness {}/{} {} repeat {}".format(
