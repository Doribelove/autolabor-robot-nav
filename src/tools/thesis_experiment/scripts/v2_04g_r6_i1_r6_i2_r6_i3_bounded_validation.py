#!/usr/bin/env python3
"""Fail-closed future runner for the bounded R6-I3 simulation schedule.

The canonical release does not exist during the execution-readiness review,
so the only currently usable mode is ``--offline-review``.  Execution mode is
implemented for a later, separately authorized turn, but it cannot create a
directory, journal, or subprocess until both caller-supplied hashes and every
release/closure binding have passed the dedicated validator.
...
MACHINE_REVIEW_RELATIVE = (
    READINESS_ROOT_RELATIVE / "v2_04g_r6_i3_execution_readiness_review.yaml"
)
ATTEMPTS_ROOT = WORKSPACE / READINESS_ROOT_RELATIVE / "attempts"
JOURNAL_ROOT = WORKSPACE / READINESS_ROOT_RELATIVE / "journals"
STAGE_REPORT = (
    WORKSPACE / READINESS_ROOT_RELATIVE / "v2_04g_r6_i3_stage_report.yaml"
)
...
I2_AUTHORIZATION_RELATIVE = Path(
    "src/tools/thesis_experiment/src/thesis_experiment/"
...
EXPECTED_RELEASE_VALIDATOR_SHA256 = (
    "c79551a518d36bd04ddec3dbc0cd99a8b5a881e41cd3edeec041bea8004ef43a"
)
TRUSTED_MODULE_SHA256 = {
    I2_AUTHORIZATION_RELATIVE: (
        "cf38fa42fb660cdca58ed456eab381358e1d4ea1d75f575fb647dc1c5fb98199"
    ),
...
class R6I3ExecutionError(RuntimeError):
    """Fail-closed R6-I3 execution error."""
...
def _read_workspace_bytes_once(relative):
    ...
    if not WORKSPACE.is_absolute() or WORKSPACE != WORKSPACE.resolve():
        raise R6I3ExecutionError("workspace root is not canonical")
    if WORKSPACE.is_symlink() or not WORKSPACE.is_dir():
        raise R6I3ExecutionError("workspace root is unsafe")
    ...
    if (
        ...
        raise R6I3ExecutionError("trusted path is unsafe: {}".format(relative))
...
def _runtime_state_present():
    return {
        "release": (WORKSPACE / RELEASE_RELATIVE).exists(),
        "attempts_root": ATTEMPTS_ROOT.exists(),
        "journals_root": JOURNAL_ROOT.exists(),
        "stage_report": STAGE_REPORT.exists(),
    }
...
def _canonical_cli_path(raw, expected_relative, label):
    if not isinstance(raw, str) or not raw:
        raise R6I3ExecutionError("{} path is required".format(label))
    candidate = Path(raw)
    if not candidate.is_absolute():
        candidate = WORKSPACE / candidate
    if candidate.absolute() != (WORKSPACE / expected_relative).absolute():
        raise R6I3ExecutionError("{} path drifted".format(label))
...
def _execution_preflight(
    ...
    if not _is_sha256(release_sha256):
        raise R6I3ExecutionError("caller release SHA256 is invalid")
    if authorization_sha256 != EXPECTED_AUTHORIZATION_SHA256:
        raise R6I3ExecutionError("caller authorization SHA256 drifted")
    ...
    release_module = _load_verified_module(
        ...
        "thesis_experiment.v2_04g_r6_i1_r6_i2_r6_i3_release",
    )
    preloaded_snapshots = {
    ...
    if not _exact(preregistration.get("schedule"), EXPECTED_SCHEDULE):
        raise R6I3ExecutionError("validated exact schedule drifted")
    if validation.schedule_sha256 != EXPECTED_SCHEDULE_SHA256:
        raise R6I3ExecutionError("validated schedule digest drifted")
    if (
        validation.identity_count != 6
        or tuple(validation.execution_seeds) != (5151, 5152, 5153)
    ):
        raise R6I3ExecutionError("validated seed/budget boundary drifted")
...
def _spawn_sanitized(
    ...
    try:
        ...
        thread = threading.Thread(
            target=drain,
            name="r6_i3_sanitized_process_log",
            daemon=True,
        )
        thread.start()
        return {
...
def _stop_process(handle, timeout_s=12.0):
    ...
    if handle["drain_errors"]:
        raise R6I3ExecutionError("sanitized process-log drain failed")
...
def _clock_sample(payload):
    ...
    if sec is None or nsec is None:
        raise R6I3ExecutionError("/clock probe output is malformed")
...
def _bootstrap_clock_and_services(
    ...
    if not barrier.service_wait_allowed:
        raise R6I3ExecutionError("positive /clock barrier did not release")
    _wait_for_service(
        base_handle,
        environment,
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if result is not None:
                ...
                return
            time.sleep(0.1)
        raise R6I3ExecutionError("action trace readiness timed out")
    finally:
...
def _validate_semantics(row, activation, evaluation, minimum, integrity):
    ...
    if not (
        ...
        raise R6I3ExecutionError("R6-I3 readiness/evaluator gate failed")
    ...
    if role in {"non_none", "non_none_iff_finite_ttc"}:
        ...
        raise R6I3ExecutionError("unknown overlay semantic role")
...
def _run_attempt(row, context, ledger_entry, report, source_environment):
    ...
    if not (
        ...
        raise R6I3ExecutionError("compiled execution scene identity drifted")
    ...
    with integrity.AtomicAttemptJournal(JOURNAL_ROOT, identity) as journal:
        ...
        try:
            ...
            if hashlib.sha256(startup_payload).hexdigest() != initial[
                ...
                raise R6I3ExecutionError("initial profile hash drifted")
            ...
            if not (
                ...
                raise R6I3ExecutionError("arm provenance mismatched")
            ...
            while True:
                ...
                if time.monotonic() > deadline:
                    raise R6I3ExecutionError("episode deadline exceeded")
                time.sleep(0.1)
            ...
            except R6I3ExecutionError as exc:
            ...
            for handle in (listener, episode, transaction, base):
                try:
                    ...
                    pass
            raise R6I3ExecutionError(safe_reason) from None
...
def _execute_validated(
    ...
    for row in context["preregistration"]["schedule"]:
        ...
        try:
            ...
            report.update({
                ...
                "unattempted_budget_forfeited": 6 - consumed,
                "resume_forbidden": True,
            })
            _atomic_yaml(STAGE_REPORT, report, yaml_module)
            if _process_matches():
    ...
    print(yaml_module.safe_dump(report, sort_keys=False))
    return 0
...
def execute(
    ...
    try:
        ...
        safe_reason = (
            "R6-I3 execution failed closed; inspect only sanitized evidence "
