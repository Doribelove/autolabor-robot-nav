#!/usr/bin/env python3
"""Run the frozen R6 listener with an R6-I5 evidence identity.
...
def _read_frozen_source_once():
    ...
    try:
        ...
        if not stat.S_ISREG(before.st_mode) or before.st_size > MAX_SOURCE_BYTES:
            raise R6I5WrapperError("frozen listener source is unsafe")
        chunks = []
        ...
        if not (
            ...
            raise R6I5WrapperError("frozen listener changed during read")
        if hashlib.sha256(payload).hexdigest() != SOURCE_SHA256:
            raise R6I5WrapperError("frozen listener SHA256 drifted")
        return payload
...
def _load_frozen_listener():
    ...
    name = "v2_04g_r6_i1_listener_frozen_for_r6_i5"
