#!/usr/bin/env python3
"""Run the frozen R6 mechanism episode with an R6-I3 identity."""
...
def _read_frozen_source_once():
    ...
    try:
        ...
        if not stat.S_ISREG(before.st_mode) or before.st_size > MAX_SOURCE_BYTES:
            raise R6I3WrapperError("frozen episode source is unsafe")
        chunks = []
        ...
        if not (
            ...
            raise R6I3WrapperError("frozen episode changed during read")
        if hashlib.sha256(payload).hexdigest() != SOURCE_SHA256:
            raise R6I3WrapperError("frozen episode SHA256 drifted")
        return payload
...
def _load_frozen_episode():
    ...
    name = "v2_04g_r6_i1_episode_frozen_for_r6_i3"
