#!/usr/bin/env python3
"""Run the preregistered R4 readiness gate before TTC and navigation."""
...
_SPEC = importlib.util.spec_from_file_location("v2_04g_r3_frozen_probe_helpers_r4", R3_BATCH)
...
def verify_resources(preregistration):
    for group in ("resources", "frozen_repair_boundary"):
        for name, resource in preregistration.get(group, {}).items():
            ...
            if not path.is_file() or _R3._sha256(path) != resource["sha256"]:
...
def _summary(preregistration_path, schedule, reports, status, failure):
    summary = _R3._summary(preregistration_path, schedule, reports, status, failure)
    summary["stage"] = STAGE
    mismatch_count = sum(
        report.get("world_model_sequence_mismatch_count", 0) for report in reports
    )
    input_join_fault_count = sum(
        report.get("world_model_input_join_fault_count", 0) for report in reports
    )
    atomic_pass = mismatch_count == 0 and input_join_fault_count == 0
    summary.update({
        "world_model_sequence_mismatch_count": mismatch_count,
        "world_model_input_join_fault_count": input_join_fault_count,
        "atomic_world_model_input_alignment_pass": atomic_pass,
        "all_probe_hard_gates_pass": (
            summary["all_probe_hard_gates_pass"] and atomic_pass
        ),
        "ttc_started": False,
        "navigation_started": False,
    })
    summary["readiness_pass"] = summary["all_probe_hard_gates_pass"]
    summary["ttc_probe_authorized"] = summary["all_probe_hard_gates_pass"]
    summary["navigation_authorized"] = summary["all_probe_hard_gates_pass"]
    return summary
...
def main():
    ...
    parser.add_argument("--listener", type=Path, default=Path(__file__).with_name(
        "v2_04g_r4_activation_probe_listener.py"))
    ...
    if not (
        ...
        raise ValueError("R4 readiness boundary drifted")
    ...
    if (args.compiled_scenes_dir / "compiled_scene_index.yaml").resolve() != expected_index.resolve():
        raise ValueError("R4 compiled scene path drifted")
    output_root = args.output_root.resolve()
    output_root.relative_to((WORKSPACE /
        "artifacts/v2/calibration/v2_04g_r4/activation_probe").resolve())
    ...
    if len(schedule) != prereg["budget"]["activation_readiness_probe_count"]:
        raise ValueError("R4 readiness budget drifted")
    if {row["seed"] for row in schedule} != set(
        prereg["seed_firewall"]["readiness_probe_only_seeds"]
    ):
        raise ValueError("R4 readiness seed schedule drifted")
    reports = []
    ...
    for row in schedule:
        ...
        if profile not in probe["profile_ids"]:
            raise ValueError("R4 readiness profile schedule drifted")
        ...
        if report_path.is_file():
            ...
            if not all(report.get(key) == value for key, value in identity.items()):
                raise RuntimeError("existing R4 readiness identity drifted")
            if report.get("all_hard_gates_pass") is not True:
                raise RuntimeError("existing R4 readiness probe is failed evidence")
        ...
        if not report_path.is_file():
            ...
            _R3._write_yaml(summary_path, _summary(
        ...
        if (result is None or result.returncode != 0
            ...
            failure = {**row, "reason": "R4 atomic-input readiness hard gate failed"}
            _R3._write_yaml(summary_path, _summary(
                args.preregistration, schedule, reports, "failed", failure))
            raise RuntimeError(failure["reason"])
        print("PASS R4 readiness {}/{} {} repeat {}".format(
