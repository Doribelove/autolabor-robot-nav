#!/usr/bin/env python3
"""Fail-closed assessment for R4 full calibration and winner eligibility."""
...
STAGE = "V2-04G-R4"
R3_ASSESSOR = Path(__file__).with_name("assess_v2_04g_r3.py")
_SPEC = importlib.util.spec_from_file_location("assess_v2_04g_frozen_for_r4", R3_ASSESSOR)
...
def _load_r4(progress_path):
    ...
    if not (
        ...
        raise ValueError("V2-04G-R4 navigation evidence is incomplete")
    rows = {}
    for record in progress["episodes"]:
        path = Path(record["evaluation"])
        if _sha256(path) != record["evaluation_sha256"]:
            raise ValueError("V2-04G-R4 evaluation hash drifted")
        value = yaml.safe_load(path.read_text(encoding="utf-8"))
        if value.get("stage") != STAGE or value.get("split") != "calibration":
            raise ValueError("V2-04G-R4 evaluation boundary drifted")
        trace_path = path.parent / "trace.csv"
        load_v2_trace(trace_path)
        if trace_sha256(trace_path) != value["raw_trace_sha256"]:
            raise ValueError("V2-04G-R4 trace hash drifted")
        key = (value["supervisor_profile_id"], value["scene_id"])
        if key in rows:
            raise ValueError("duplicate V2-04G-R4 evidence identity")
        rows[key] = value
...
def _activation_pass(summary_path, preregistration_path, planned_count):
    ...
    taxonomy = summary.get("aggregate_fault_taxonomy_counts", {})
    return bool(
        ...
        and summary.get("world_model_sequence_mismatch_count") == 0
        and summary.get("world_model_input_join_fault_count") == 0
        and summary.get("atomic_world_model_input_alignment_pass") is True
        and taxonomy.get("BACKEND_TRANSACTION_FAULT", 0) == 0
        and taxonomy.get("UNKNOWN_TRANSACTION_FAULT", 0) == 0
...
def assess(progress_path, prereg_path, contract_path, ttc_probe_path, activation_path):
    ...
    if prereg.get("stage") != STAGE or contract.get("stage") != STAGE:
        raise ValueError("V2-04G-R4 assessment boundary drifted")
    if _sha256(contract_path) != prereg["resources"]["contract"]["sha256"]:
        raise ValueError("V2-04G-R4 contract hash drifted")
    ...
    frozen_load, frozen_candidates = _LEGACY._load, _LEGACY.CANDIDATES
    _LEGACY._load, _LEGACY.CANDIDATES = _load_r4, candidates
    try:
        report = _LEGACY.assess(
            progress_path, prereg_path, contract_path, ttc_probe_path)
    finally:
        _LEGACY._load, _LEGACY.CANDIDATES = frozen_load, frozen_candidates
    _, evidence = _load_r4(progress_path)
    ...
    report.update({
        "stage": STAGE, "activation_readiness_probe_pass": activation_ok,
        "passing_candidate_ids": passing, "winner_candidate_id": winner,
        "all_hard_gates_pass": winner is not None,
    })
...
def main():
    ...
    parser.add_argument("--progress", type=Path, default=WORKSPACE /
        "artifacts/v2/calibration/v2_04g_r4/v2_04g_r4_progress.yaml")
    parser.add_argument("--preregistration", type=Path, default=WORKSPACE /
        "experiments/manifests/v2/calibration/v2_04g_r4_preregistration.yaml")
    parser.add_argument("--contract", type=Path, default=WORKSPACE /
        "config/thesis_experiments/v2/v2_04g_r4_full_calibration_contract.yaml")
    parser.add_argument("--ttc-probe", type=Path, default=WORKSPACE /
        "artifacts/v2/calibration/v2_04g_r4/v2_04g_r4_ttc_three_state_probe.yaml")
    parser.add_argument("--activation-probe", type=Path, default=WORKSPACE /
        "artifacts/v2/calibration/v2_04g_r4/activation_probe/activation_probe_summary.yaml")
    parser.add_argument("--output", type=Path, default=WORKSPACE /
        "artifacts/v2/calibration/v2_04g_r4/v2_04g_r4_assessment.yaml")
