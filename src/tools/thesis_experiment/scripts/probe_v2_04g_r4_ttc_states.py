#!/usr/bin/env python3
"""Produce the frozen three-state TTC component probe for V2-04G-R4."""
...
from nav_world_model.risk_evidence import RelativeTrack, classify_ttc_evidence, earliest_relative_ttc
...
def main():
    ...
    if not (prereg.get("stage") == "V2-04G-R4"
            and prereg.get("split") == "calibration"
            and prereg.get("training_allowed") is False):
        raise ValueError("TTC probe requires R4 calibration preregistration")
    readiness_path = Path(prereg["activation_readiness_probe"]["summary_path"])
    if not readiness_path.is_absolute():
        readiness_path = Path("/home/robot/robot_ws_base_rl") / readiness_path
    readiness = yaml.safe_load(readiness_path.read_text(encoding="utf-8"))
    if not (readiness.get("stage") == "V2-04G-R4"
            and readiness.get("all_probe_hard_gates_pass") is True
            and readiness.get("preregistration", {}).get("sha256") == _sha256(args.preregistration)):
        raise RuntimeError("R4 TTC probe refused before readiness pass")
    ...
    probes = [
        {"probe_id": "healthy_observed_conflict", "tracker_message_count": 1,
         "healthy_tracker_sample_count": 1, "finite_ttc_sample_count": int(conflict is not None),
         "finite_ttc_s": conflict},
        {"probe_id": "healthy_no_conflict_in_horizon", "tracker_message_count": 1,
         "healthy_tracker_sample_count": 1, "finite_ttc_sample_count": int(clear is not None),
         "finite_ttc_s": clear},
        {"probe_id": "tracker_invalid_fail_closed", "tracker_message_count": 0,
         "healthy_tracker_sample_count": 0, "finite_ttc_sample_count": 0, "finite_ttc_s": None},
    ]
    for probe in probes:
    ...
    report = {
        ...
        "probe_count": 3, "expected_status_order": expected,
        "observed_status_order": observed, "all_three_states_pass": observed == expected,
        "probes": probes,
    }
