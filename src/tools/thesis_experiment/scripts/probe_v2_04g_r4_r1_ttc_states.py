#!/usr/bin/env python3
"""Produce the frozen three-state TTC probe for V2-04G-R4-R1."""
...
def main():
    ...
    if not (prereg.get("stage") == STAGE
            and prereg.get("split") == "calibration"
            and prereg.get("training_allowed") is False):
        raise ValueError("TTC probe requires R4-R1 calibration preregistration")
    readiness_path = WORKSPACE / prereg["activation_readiness_probe"]["summary_path"]
    readiness = yaml.safe_load(readiness_path.read_text(encoding="utf-8"))
    if not (readiness.get("stage") == STAGE
            and readiness.get("all_probe_hard_gates_pass") is True
            and readiness.get("preregistration", {}).get("sha256")
            == _sha256(args.preregistration)):
        raise RuntimeError("R4-R1 TTC probe refused before readiness pass")
    ...
    report = {
        ...
        "readiness_summary": {"path": str(readiness_path),
