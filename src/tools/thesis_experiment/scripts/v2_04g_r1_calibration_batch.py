#!/usr/bin/env python3
"""Run/resume preregistered V2-04G-R1 interface-repair calibration."""
...
def _verify_preregistered_resources(preregistration):
    for name, resource in preregistration.get("resources", {}).items():
        ...
        if not path.is_file() or _sha256(path) != resource["sha256"]:
            raise ValueError("V2-04G-R1 resource hash drifted: {}".format(name))
    for name, resource in preregistration.get(
        "frozen_v2_04g_stop_boundary", {}
    ).items():
        if name == "reused_as_v2_04g_r1_navigation_evidence":
            continue
        path = WORKSPACE / resource["path"]
        if not path.is_file() or _sha256(path) != resource["sha256"]:
            raise ValueError("frozen V2-04G stop evidence drifted: {}".format(name))


def _require_activation_probe(preregistration, preregistration_path):
    probe = preregistration.get("activation_readiness_probe", {})
    if probe.get("required_before_navigation") is not True:
        raise ValueError("V2-04G-R1 activation probe requirement drifted")
    summary_path = WORKSPACE / probe["summary_path"]
    if not summary_path.is_file():
        raise RuntimeError("activation readiness summary is missing")
    summary = yaml.safe_load(summary_path.read_text(encoding="utf-8"))
    expected_count = probe["planned_probe_count"]
    expected = {
        "stage": "V2-04G-R1", "status": "complete",
        "planned_probe_count": expected_count,
        "valid_probe_count": expected_count,
    ...
    for key, value in expected.items():
        if summary.get(key) != value:
            raise RuntimeError("activation readiness summary failed {}".format(key))
    if summary.get("preregistration", {}).get("sha256") != _sha256(
        preregistration_path
    ):
        raise RuntimeError("activation readiness summary preregistration hash drifted")
    return summary_path
...
def _materialize_candidates(candidate_path, output_dir, expected_stage):
    ...
    if expected_stage == "V2-04G-R1":
        ...
        if _sha256(supervisor_source) != bank["shared_supervisor"]["sha256"]:
            raise ValueError("V2-04G-R1 frozen supervisor source hash drifted")
        if _sha256(anchor_source) != bank["shared_anchor_bank"]["sha256"]:
            raise ValueError("V2-04G-R1 frozen Anchor Bank source hash drifted")
        ...
        for candidate in bank["candidates"]:
            ...
            supervisor["profile_id"] = "fam_teb_v2_04g_r1_{}_supervisor".format(candidate_id)
            anchor["status"] = "uncalibrated_simulation_candidate"
            anchor["bank_id"] = "fam_teb_v2_04g_r1_{}_anchor_candidate".format(candidate_id)
            anchor["source_provenance"]["mode_deltas"] = (
                "v2_04g_r1_frozen_single_factor_interface_repair_candidate"
            )
            deep_update(supervisor, candidate["supervisor_patch"])
            deep_update(anchor, candidate["anchor_patch"])
            supervisor_path = candidate_dir / "supervisor.yaml"
            anchor_path = candidate_dir / "anchor_bank.yaml"
            _write_yaml(supervisor_path, supervisor)
            _write_yaml(anchor_path, anchor)
            mechanism_path = ""
            if candidate["mechanism"] is not None:
                mechanism = copy.deepcopy(candidate["mechanism"])
                mechanism["profile_id"] = "fam_teb_v2_04g_r1_{}_mechanism".format(candidate_id)
                mechanism_target = candidate_dir / "mechanism.yaml"
                _write_yaml(mechanism_target, mechanism)
                mechanism_path = str(mechanism_target.resolve())
            paths[candidate_id] = {
    ...
    if stage in ("V2-04E", "V2-04E2", "V2-04E3", "V2-04E4"):
        ...
        for scene_id in scene_ids:
            ...
            rows.append({
                ...
                "runtime_config": str(frozen_supervisor.resolve()),
                "anchor_bank": str(frozen_anchor.resolve()), "mechanism_config": "",
                "scene_id": scene_id, "family": scene["family"], "seed": scene["seed"],
            })
        for candidate_id in preregistration["candidate_ids"]:
            if candidate_id not in runtime_configs:
                raise ValueError("V2-04G-R1 materialized candidate set drifted")
...
def run_episode(row, args, instances, environment):
    ...
    launch_command = [
        ...
        "anchor_bank:={}".format(row.get(
            "anchor_bank",
            WORKSPACE / "src/application/teb_mode_manager/config/v2_04c_anchor_bank_frozen.yaml",
        )),
        "mechanism_config:={}".format(row.get("mechanism_config", "")),
...
def main():
    ...
    parser.add_argument("--episode-runner", type=Path,
        default=WORKSPACE / "src/tools/thesis_experiment/scripts/v2_04g_r1_mechanism_episode.py")
    ...
    if stage != "V2-04G-R1" or split != "calibration":
        raise ValueError("V2-04G-R1 batch refuses non-calibration input")
    _verify_preregistered_resources(prereg)
    _require_activation_probe(prereg, args.preregistration)
    if args.attempts_per_episode != prereg["budget"][
        "attempts_per_navigation_episode_max"
    ]:
        raise ValueError("V2-04G-R1 attempt budget drifted")
    ...
    if compiled_index.resolve() != expected_index.resolve():
        raise ValueError("V2-04G-R1 compiled scene directory drifted")
    runtime_configs = {}
    if stage == "V2-04G-R1":
        if args.candidate_bank is None:
            raise ValueError("V2-04G-R1 requires --candidate-bank")
        if args.candidate_bank.resolve() != (
            WORKSPACE / prereg["resources"]["candidate_bank"]["path"]
        ).resolve():
            raise ValueError("V2-04G-R1 candidate bank path drifted")
