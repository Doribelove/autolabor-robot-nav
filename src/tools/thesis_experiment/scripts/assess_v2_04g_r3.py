#!/usr/bin/env python3
"""Fail-closed assessment for V2-04G-R3 full calibration-only evidence."""
...
_SPEC = importlib.util.spec_from_file_location("assess_v2_04g_frozen_for_r3", LEGACY_ASSESSOR)
...
def _load_r3(progress_path):
    ...
    if not (
        ...
        raise ValueError("V2-04G-R3 navigation evidence is incomplete")
    rows = {}
    for record in progress["episodes"]:
        path = Path(record["evaluation"])
        if _sha256(path) != record["evaluation_sha256"]:
            raise ValueError("V2-04G-R3 evaluation hash drifted")
        value = yaml.safe_load(path.read_text(encoding="utf-8"))
        if value.get("stage") != "V2-04G-R3" or value.get("split") != "calibration":
            raise ValueError("V2-04G-R3 evaluation boundary drifted")
        trace_path = path.parent / "trace.csv"
        load_v2_trace(trace_path)
        if trace_sha256(trace_path) != value["raw_trace_sha256"]:
            raise ValueError("V2-04G-R3 trace hash drifted")
        key = (value["supervisor_profile_id"], value["scene_id"])
        if key in rows:
            raise ValueError("duplicate V2-04G-R3 evidence identity")
        rows[key] = value
...
def assess(progress_path, prereg_path, contract_path, ttc_probe_path, activation_path):
    ...
    if prereg.get("stage") != "V2-04G-R3" or contract.get("stage") != "V2-04G-R3":
        raise ValueError("V2-04G-R3 assessment boundary drifted")
    ...
    _LEGACY._load, _LEGACY.CANDIDATES = _load_r3, candidates
    try:
    ...
    _, evidence = _load_r3(progress_path)
    ...
    report.update({
        "stage": "V2-04G-R3", "activation_readiness_probe_pass": activation_ok,
...
def main():
    ...
    parser.add_argument("--progress", type=Path, default=WORKSPACE /
        "artifacts/v2/calibration/v2_04g_r3/v2_04g_r3_progress.yaml")
    ...
    parser.add_argument("--ttc-probe", type=Path, default=WORKSPACE /
        "artifacts/v2/calibration/v2_04g_r3/v2_04g_r3_ttc_three_state_probe.yaml")
    parser.add_argument("--activation-probe", type=Path, default=WORKSPACE /
        "artifacts/v2/calibration/v2_04g_r3/activation_probe/activation_probe_summary.yaml")
    parser.add_argument("--output", type=Path, default=WORKSPACE /
        "artifacts/v2/calibration/v2_04g_r3/v2_04g_r3_assessment.yaml")
