#!/usr/bin/env python3
"""Run/resume the preregistered V2-04G calibration-only experiment."""
...
def _materialize_candidates(candidate_path, output_dir, expected_stage):
    ...
    if expected_stage == "V2-04G":
        ...
        if _sha256(supervisor_source) != bank["shared_supervisor"]["sha256"]:
            raise ValueError("V2-04G frozen supervisor source hash drifted")
        if _sha256(anchor_source) != bank["shared_anchor_bank"]["sha256"]:
            raise ValueError("V2-04G frozen Anchor Bank source hash drifted")
        ...
        for candidate in bank["candidates"]:
            ...
            supervisor["profile_id"] = "fam_teb_v2_04g_{}_supervisor".format(candidate_id)
            anchor["status"] = "uncalibrated_simulation_candidate"
            anchor["bank_id"] = "fam_teb_v2_04g_{}_anchor_candidate".format(candidate_id)
            anchor["source_provenance"]["mode_deltas"] = (
                "v2_04g_preregistered_calibration_only_mechanism_candidate"
            )
            ...
            if candidate["mechanism"] is not None:
                ...
                mechanism["profile_id"] = "fam_teb_v2_04g_{}_mechanism".format(candidate_id)
...
def build_schedule(preregistration, instances, runtime_configs):
    ...
    if stage in ("V2-04E", "V2-04E2", "V2-04E3", "V2-04E4"):
        ...
        for candidate_id in preregistration["candidate_ids"]:
            if candidate_id not in runtime_configs:
                raise ValueError("V2-04G materialized candidate set drifted")
...
def run_episode(row, args, instances, environment):
    ...
    launch_command = [
        "roslaunch", "m2_gazebo", "m2_v2_04g_mechanism_calibration.launch",
...
def main():
    ...
    parser.add_argument("--episode-runner", type=Path,
        default=WORKSPACE / "src/tools/thesis_experiment/scripts/v2_04g_mechanism_episode.py")
    ...
    if stage != "V2-04G" or split != "calibration":
        raise ValueError("V2-04G batch refuses non-calibration input")
    ...
    if stage == "V2-04G":
        if args.candidate_bank is None:
            raise ValueError("V2-04G requires --candidate-bank")
