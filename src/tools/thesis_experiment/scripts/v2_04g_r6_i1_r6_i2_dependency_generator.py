#!/usr/bin/env python3
"""Persist the canonical R6-I2 integration dependency closure."""
...
def main():
    ...
    canonical_output = (
        root / OUTPUT.relative_to(WORKSPACE)
    ).resolve()
    if output != canonical_output:
        parser.error("output path must be the canonical R6-I2 closure")
