...
- **`poller_rate_hz`**: 设置底盘状态完整轮询的目标频率，默认为 1 Hz。M2 驱动是周期性 VCU 状态查询的唯一所有者；依次查询硬急停、软急停、手柄急停、整车运行状态，再轮询一项电池遥测。每次只发送一个 CAN 查询，真车 launch 将平均尝试率限制为 `3.0 Hz`，并以 `status_query_jitter_fraction=0.20` 在周期上加入 ±20% 的确定性抖动，避免与 VCU 广播固定相位冲突。安全字段没有匹配回包时最多重试 4 次，然后轮转到下一字段，保证任一故障字段都不能饿死其他安全检查。
