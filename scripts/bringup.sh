...
ROBOT_WS="${ROBOT_WS:-$(cd "$SCRIPT_DIR/.." && pwd)}"
ROS_SETUP="${ROS_SETUP:-/opt/ros/noetic/setup.bash}"
WORKSPACE_SETUP="${WORKSPACE_SETUP:-$ROBOT_WS/devel/setup.bash}"
...
NAVIGATION_LAUNCH_FILE="${NAVIGATION_LAUNCH_FILE:-navigation_arena.launch}"
...
MID360_AVOIDANCE_MIN_HEIGHT="${MID360_AVOIDANCE_MIN_HEIGHT:-0.25}"
MID360_AVOIDANCE_MAX_HEIGHT="${MID360_AVOIDANCE_MAX_HEIGHT:-0.8}"
MID360_AVOIDANCE_RANGE_MIN="${MID360_AVOIDANCE_RANGE_MIN:-0.5}"
MID360_AVOIDANCE_RANGE_MAX="${MID360_AVOIDANCE_RANGE_MAX:-12.0}"
...
usage() {
  ...
  echo "  GPS_GEOFENCE_ENABLED=true      # load local GPS forbidden polygons into both costmaps"
  echo "  GPS_GEOFENCE_FILE=.../gps_geofences.yaml # persistent multi-region fence file"
  ...
  echo "  MID360_AVOIDANCE_MIN_HEIGHT=0.25"
  echo "  MID360_AVOIDANCE_MAX_HEIGHT=0.8"
  echo "  MID360_AVOIDANCE_RANGE_MIN=0.5"
  echo "  MID360_AVOIDANCE_RANGE_MAX=12.0"
}
...
start_optional_dual_lidar() {
  DUAL_LIDAR_AVAILABLE=false

  if [[ "$DUAL_LIDAR_MODE" == "off" ]]; then
    echo "==> dual LD19 avoidance: disabled; /scan will use MID360 only"
    return 0
  fi

  local missing_ports=()
  [[ -e "$DUAL_LIDAR_FRONT_PORT" ]] || missing_ports+=("front=$DUAL_LIDAR_FRONT_PORT")
  [[ -e "$DUAL_LIDAR_REAR_PORT" ]] || missing_ports+=("rear=$DUAL_LIDAR_REAR_PORT")
  if (( ${#missing_ports[@]} )); then
    if [[ "$DUAL_LIDAR_MODE" == "on" ]]; then
      echo "Dual LD19 mode is forced on, but a configured device is missing: ${missing_ports[*]}" >&2
      return 1
    fi
    echo "==> dual LD19 avoidance: not connected; /scan will use MID360 only"
    return 0
  fi

  local front_device
  local rear_device
  front_device="$(readlink -f "$DUAL_LIDAR_FRONT_PORT")"
  rear_device="$(readlink -f "$DUAL_LIDAR_REAR_PORT")"
  if [[ "$front_device" == "$rear_device" ]]; then
    echo "Front and rear LD19 paths resolve to the same device: $front_device" >&2
    if [[ "$DUAL_LIDAR_MODE" == "on" ]]; then
      return 1
    fi
    echo "==> dual LD19 avoidance: invalid port pair; /scan will use MID360 only"
    return 0
  fi

  if [[ ! -w "$DUAL_LIDAR_FRONT_PORT" || ! -w "$DUAL_LIDAR_REAR_PORT" ]]; then
    if [[ "$DUAL_LIDAR_MODE" == "on" ]]; then
      make_writable "$DUAL_LIDAR_FRONT_PORT"
      make_writable "$DUAL_LIDAR_REAR_PORT"
    else
      echo "==> dual LD19 avoidance: USB ports are not writable; /scan will use MID360 only"
      echo "    Add the user to dialout or set DUAL_LIDAR_MODE=on after fixing permissions." >&2
      return 0
    fi
  fi

  echo "==> dual LD19 avoidance: connected (front=$front_device, rear=$rear_device)"
  start_launch "Optional front/rear LD19 avoidance scan" \
    autolabor_dual_lidar dual_ld19_fused.launch \
    front_port:="$DUAL_LIDAR_FRONT_PORT" \
    rear_port:="$DUAL_LIDAR_REAR_PORT" \
    lidar_center_distance_m:="$DUAL_LIDAR_CENTER_DISTANCE_M" \
    front_topic:=/dual_lidar/front/scan_raw \
    rear_topic:=/dual_lidar/rear/scan_raw \
    output_topic:=/dual_lidar/scan \
  ...
  if wait_topics "/dual_lidar/scan" 15.0; then
    DUAL_LIDAR_AVAILABLE=true
    echo "==> dual LD19 avoidance: live on /dual_lidar/scan"
    return 0
  fi

  echo "==> dual LD19 avoidance: no fused scan; /scan will use MID360 only" >&2
  if [[ "$DUAL_LIDAR_MODE" == "on" ]]; then
    return 1
  fi
}
...
start_optional_cloud_enhancer() {
  if [[ "$DUAL_LIDAR_AVAILABLE" != "true" ]]; then
    return 0
  fi

  start_launch "Optional dual LD19 visualization cloud" \
    autolabor_dual_lidar cloud_enhancer_only.launch \
    lidar_scan_topic:=/dual_lidar/scan \
    enhanced_cloud_topic:="$DUAL_LIDAR_ENHANCED_CLOUD_TOPIC"

  if wait_topics "$DUAL_LIDAR_ENHANCED_CLOUD_TOPIC" 10.0; then
    echo "==> visualization cloud: live on $DUAL_LIDAR_ENHANCED_CLOUD_TOPIC"
  else
    echo "==> visualization cloud did not start; avoidance /scan is unaffected" >&2
  fi
}

start_independent_avoidance_scan() {
  start_launch "Independent MID360 and optional dual-LD19 avoidance" \
    robot_bringup scan_mid360_avoidance.launch \
    use_dual_lidar:="$DUAL_LIDAR_USE_FOR_SCAN" \
    sensor_x:="$MID360_AVOIDANCE_SENSOR_X" \
    sensor_y:="$MID360_AVOIDANCE_SENSOR_Y" \
    sensor_z:="$MID360_AVOIDANCE_SENSOR_Z" \
    sensor_roll:="$MID360_AVOIDANCE_SENSOR_ROLL" \
    sensor_pitch:="$MID360_AVOIDANCE_SENSOR_PITCH" \
    sensor_yaw:="$MID360_AVOIDANCE_SENSOR_YAW" \
    min_height:="$MID360_AVOIDANCE_MIN_HEIGHT" \
    max_height:="$MID360_AVOIDANCE_MAX_HEIGHT" \
    range_min:="$MID360_AVOIDANCE_RANGE_MIN" \
    range_max:="$MID360_AVOIDANCE_RANGE_MAX"

  wait_topics "/mid360/points_avoidance,/mid360/scan,/scan" 45.0
  if [[ "$DUAL_LIDAR_USE_FOR_SCAN" == "true" ]]; then
    echo "==> MoveBase avoidance: raw MID360 + fresh optional LD19 -> /scan"
  else
    echo "==> MoveBase avoidance: raw MID360 only -> /scan (LD19 fusion disabled)"
  fi
}
...
case "$DUAL_LIDAR_MODE" in
  auto|on|off) ;;
  *)
    echo "Invalid DUAL_LIDAR_MODE: $DUAL_LIDAR_MODE (use auto, on, or off)" >&2
    exit 1
    ;;
esac

case "$DUAL_LIDAR_USE_FOR_SCAN" in
  true|false) ;;
  *)
    echo "Invalid DUAL_LIDAR_USE_FOR_SCAN: $DUAL_LIDAR_USE_FOR_SCAN (use true or false)" >&2
    exit 1
    ;;
esac

if ! is_positive_number "$DUAL_LIDAR_CENTER_DISTANCE_M"; then
  echo "Invalid DUAL_LIDAR_CENTER_DISTANCE_M: $DUAL_LIDAR_CENTER_DISTANCE_M" >&2
  exit 1
fi

for value_name in \
  MID360_AVOIDANCE_SENSOR_X MID360_AVOIDANCE_SENSOR_Y MID360_AVOIDANCE_SENSOR_Z \
  MID360_AVOIDANCE_SENSOR_ROLL MID360_AVOIDANCE_SENSOR_PITCH MID360_AVOIDANCE_SENSOR_YAW \
  MID360_AVOIDANCE_MIN_HEIGHT MID360_AVOIDANCE_MAX_HEIGHT \
  MID360_AVOIDANCE_RANGE_MIN MID360_AVOIDANCE_RANGE_MAX; do
  if ! is_number "${!value_name}"; then
    echo "Invalid $value_name: ${!value_name}" >&2
    exit 1
  fi
done

if ! awk -v min_h="$MID360_AVOIDANCE_MIN_HEIGHT" -v max_h="$MID360_AVOIDANCE_MAX_HEIGHT" \
         -v min_r="$MID360_AVOIDANCE_RANGE_MIN" -v max_r="$MID360_AVOIDANCE_RANGE_MAX" \
         'BEGIN { exit !(min_h < max_h && min_r >= 0.0 && min_r < max_r) }'; then
  echo "Invalid MID360 avoidance height/range limits." >&2
  exit 1
fi
...
if [[ "$MODE" == "gps" ]]; then
  ...
  case "$GPS_GEOFENCE_ENABLED" in
    true|false) ;;
    *)
      echo "Invalid GPS_GEOFENCE_ENABLED: $GPS_GEOFENCE_ENABLED (use true or false)" >&2
      exit 2
      ;;
  esac
  ...
  if [[ "$GPS_GEOFENCE_ENABLED" == "true" ]]; then
    require_file "$GPS_GEOFENCE_FILE"
  fi
  ...
  if [[ "$GPS_GEOFENCE_ENABLED" == "true" ]]; then
    echo "==> GPS electronic fence: enabled ($GPS_GEOFENCE_FILE)"
  else
    echo "==> GPS electronic fence: disabled"
  fi
  if [[ "$FOD_RECOVERY_STANDBY_ENABLED" == "true" ]]; then
...
if [[ -n "$LIVOX_CONFIG_FILE" ]]; then
  require_file "$LIVOX_CONFIG_FILE"
fi
...
if [[ "$MODE" == "fast_lio" || "$MODE" == "fast_lio_gps" ]]; then
  if [[ -n "$LIVOX_CONFIG_FILE" ]]; then
    start_launch "Livox Mid-360 driver" robot_bringup livox_mid360.launch user_config_path:="$LIVOX_CONFIG_FILE"
  else
  ...
  start_launch "Arena navigation" robot_bringup "$NAVIGATION_LAUNCH_FILE" \
  ...
  start_fast_lio_launch "FAST_LIO point cloud registration" robot_bringup fast_lio.launch \
    odom_pub_en:=false \
    tf_pub_en:=false \
  ...
  start_launch "Arena navigation" robot_bringup "$NAVIGATION_LAUNCH_FILE" \
    ...
    gps_geofence_enabled:="$GPS_GEOFENCE_ENABLED" \
    gps_geofence_file:="$GPS_GEOFENCE_FILE" \
