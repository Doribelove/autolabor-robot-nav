...
ROOT="$W/artifacts/t12/residual_anchor_init_pilot"
CONFIG="$W/config/thesis_experiments/t12_residual_anchor_init_pilot.yaml"
cd "$W"
...
ok=(r.get('task')=='T12' and r.get('passed') is True and
    ...
    configuration.get('residual_config_sha256')==expected and
    configuration.get('initialize_residual_anchor') is True)
raise SystemExit(0 if ok else 1)
PY
}

for seed in 101 102; do
  id="t12_residual_anchor_init_seed${seed}"
  out="$ROOT/runs/$id"
...
from pathlib import Path
root=Path('/home/robot/robot_ws_base_rl/artifacts/t12/residual_anchor_init_pilot')
config=Path('/home/robot/robot_ws_base_rl/config/thesis_experiments/t12_residual_anchor_init_pilot.yaml')
...
runs={}
for seed in (101,102):
 run=root/'runs'/f't12_residual_anchor_init_seed{seed}'
 ...
 train_steps=[row for row in steps if row['scene_split']=='train']
 ...
 first=train_steps[0]
 anchor_l1=sum(abs(float(first[f'previous_theta_{name}'])-value)/radius for name,value,radius in (
  ('max_vel_x',0.9,0.35),('max_vel_theta',0.75,0.25),('acc_lim_x',0.6,0.30),
  ('acc_lim_theta',0.6,0.30),('min_obstacle_dist',0.375,0.25),
  ('inflation_dist',0.725,0.20),('weight_obstacle',75.0,0.30),
  ('weight_viapoint',2.5,0.25),('weight_optimaltime',0.75,0.25)))
 ...
 runs[seed]={
  ...
  'initialize_residual_anchor':configuration.get('initialize_residual_anchor'),
  'residual_config_sha256_matches':configuration.get('residual_config_sha256')==expected_config_sha,
  'first_training_step_previous_theta_to_anchor_normalized_l1':anchor_l1,
...
gates={
 ...
 'anchor_initialization_active':all(item['initialize_residual_anchor'] is True for item in runs.values()),
 'first_training_step_matches_anchor':all(item['first_training_step_previous_theta_to_anchor_normalized_l1']<=1e-8 for item in runs.values()),
 'curriculum_scene_rotation_preserved':all(set(item['training_scene_ids'])==expected_scenes for item in runs.values()),
...
report={'schema_version':'1.0','task':'T12','study':'residual_episode_anchor_initialization_single_factor_pilot',
 'formal_result':False,'expanded_budget':False,'runs':runs,'gates':gates,
 'integrity_passed':integrity,'learning_gate_passed':learning,
 'decision':('frozen_paired_baseline_confirmation_then_budget_expansion_review' if integrity and learning else
             'stop_without_budget_expansion_and_review_one_next_factor')}
output=root/'t12_residual_anchor_init_pilot_report.yaml'
