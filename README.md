...
- NVIDIA 端 22 个包已用 Release 配置编译通过；当前测试结果汇总为 647 项通过、
  0 错误、0 失败、0 跳过。
...
- J6M `current` 当前指向版本 `20260826_165211`，已通过远端静态健康检查。
...
有效宽度、重叠率和转场倒车选项参与轨迹生成；计划进入 `READY` 后若修改它们，必须先
“取消已生成轨迹”并重新生成，不能把界面上的新数值当作已经应用。最高前进速度在任务
启动时单独下发，可在点击开始前调整。队列模式没有预先生成整批轨迹，开始时会统一冻结
当时显示的全部参数。
...
规划始终要求当前位姿，并按当前位置可达连通域重新裁剪。角度评分同时考虑有效扫掠长度、
转弯数量、碎片化和不可覆盖面积，选择代价最低的候选；青色预览只画最终清扫条带，不把
条带间的几何连线冒充成已经验证的可行路线。
