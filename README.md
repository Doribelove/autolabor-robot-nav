...
- J6M `current` 当前实测指向版本 `20260904_140758`；本轮固定档位 TEB、直接 Hybrid A*
  ...
  已确认已部署版本的首线 Navfn 分支错误复用了 `2 m` Hybrid 前视；权威本地源码已改为
  继承普通导航的 `4 m` 基线，但该单点修复尚未部署到 J6M。NVIDIA/J6M
  构建与远端安装态健康检查均通过；关键源码与 install-space SHA-256 一致，时钟中点偏差
  `-3 ms`。2026-09-04 曾用 `latest` 地图完整冷启动并通过运行态健康检查；操作员设置初始
  位姿后定位为 `LOCALIZED`。当前架构两轮实车覆盖均暂停：首轮运行到第 3 条线前，第二轮
  换到 41.485 m² 的更空旷区域后在线 1 -> 线 2 的首次转场复现。两轮 Hybrid 搜索本身均
  成功，TEB 固定倒车段连续产生反号候选并被安全门归零，偏离事件重规划后的候选又被旧版
  1.20 m防前进绕圈门拒绝。该后置前进距离门现已在源码删除；恢复仍受 4.0 m总长、碰撞、
  曲率和入口合同约束。第二轮实际清扫顺序及逐线坐标已经记录；受限低速实车验收仍未通过。
- 当前本地覆盖源码不再预计算任务级可执行 Hybrid 全局轨迹：首入口先限制在静态已知自由
  ...
  控制链源码已完成适配；J6M 控制链已随 release `20260904_140758` 部署。与仅把区内
...
4. 检查青色覆盖条带、可覆盖面积、不可覆盖估算和预计总时间。界面不再把条带之间的
   ...
   `0.5 s`。界面保留的 Hybrid 搜索失败冷却参数默认为 `1.0 s`，可在 `1–10 s`
   内调节；生产事件模式不会按该周期主动替换有效缓存。连续 `2.0 s` 移动不足
   `0.10 m` 才判为无进展；异常 action 已取消并确认终态后，固定退避 `2.0 s` 再从
   实时位姿重试。
...
处理。任务在精确取消并确认当前 action 终态后等待 `2 s`，最多重试 `3` 次；Hybrid
执行连续 `2 s` 移动不足 `0.10 m` 也会触发同一事件恢复。被阻断的是扫掠线时可
