...
- NVIDIA 端 22 个包已用 Release 配置编译通过；当前测试汇总为 451 项通过、0 失败。
...
- J6M 版本 `20260822_182346` 已部署并通过远端静态健康检查。
...
Navfn；强制直线路径逐姿态核对完整导航 footprint 和终点航向。转场可选让 TEB 尝试
`0.15 m/s` 低速倒车，覆盖直线禁止倒车。
...
不一致时拒绝发车。当前条带排序也计入车辆入场航向，但转场仍是 Navfn + TEB，尚未实现
Dubins/Reeds-Shepp 全局连接曲线，因此软件不会把“路径已生成”表述为整条路线已经通过
实体车运动学证明；仍需低速实车验收。
...
最近 `120` 个有效 `/Odometry` 位姿采样，便于观察最近一段行驶轨迹。Qt 明确区分“覆盖
...
车辆静止、完整障碍融合、静态/未知区碰撞策略和 move_base action server，任何一项
失败都不会发车。
