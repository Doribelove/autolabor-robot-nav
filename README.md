...
- NVIDIA 端 23 个包已用 Release 配置编译通过；当前静态检查结果汇总为 797 项通过、
  0 错误、0 失败、0 跳过。
...
- J6M `current` 当前指向版本 `20260829_170203`。local costmap 已恢复为
  `20 × 20 m`、`0.10 m` 分辨率、`10/11 m` 障碍标记/射线清除距离，静态 TEB 前视恢复为
  `8 m`，并启用 `control_look_ahead_poses=2` 的两步控制平均。NVIDIA/J6M 构建、远端静态
  检查和 fused 地图完整冷启动均通过，实时 ROS 参数已逐项核对；定位当前按设计处于
  `WAITING_INITIAL_POSE`，连续零速输出为零，本轮未发送目标或做运动验收。
...
- 三地图建图、跨帧静态过滤和地图同步已完成 bag 验证；当前 `latest` 为
  `map_20260822_slice_selfcrop_swept_final`，并已同步到 J6M。已知三维图定位采用 FAST-LIO
  高频里程计加低频多尺度 ICP。旧版覆盖清扫 V1 已完成静态地图冷启动、Qt 目视和低速
  实车分段验收；0830 分支现已在本地把覆盖专用转场升级为“Hybrid A* → TEB”，普通点到点
  仍使用 Navfn，精确扫掠仍强制跟踪清扫线。新架构已通过本地编译和离线测试，但尚未部署
  J6M、尚未做低速实车验收；旧版现场整轮任务曾因人群封路、雷达被遮挡和操作员遥控接管
  而安全暂停。
...
local costmap 已恢复为 `20 × 20 m`，障碍标记/射线清除距离为 `10/11 m`；TEB 全局计划
前视恢复为 `8 m`，低于该滚动窗口对应的覆盖安全门上限 `8.5 m`。静态导航额外以
...
4. 检查青色覆盖条带、可覆盖面积、不可覆盖估算和预计总时间。界面不再把条带之间的
   直线连接画成可执行路径；转场的实际蓝/红路线由 Hybrid A* + TEB 在执行时生成。默认有效
   宽度为 `1.00 m`、重叠率 `15%`，所以默认车道中心距为 `0.85 m`。规划参数还包括前进
   速度 `0.80 m/s`、倒车速度 `0.30 m/s`、最大转弯角速度 `0.60 rad/s`、最大线/角
   加速度 `2.00 m/s² / 0.50 rad/s²`、换向附加时间 `1.0 s` 和每段交接附加时间
   `0.5 s`。最高前进速度可在 `0.10–1.60 m/s` 内选择；NVIDIA 最终看门狗的
...
上。每次在 Qt 修改参数都会立即通过 `QSettings` 写入 NVIDIA 当前用户配置，并在下次
启动 Qt 时作为默认值。下一次 Qt 单区规划或队列启动直接冻结当前界面的完整参数组；AI
清扫若没有显式指定某一参数，也会在提交任务时读取同一份最新 `QSettings`，不再使用一套
独立的 `0.8 m/s` 等固定默认值。显式 AI 参数只覆盖对应字段。该偏好不是地图区域记录的
一部分，也不会静默改写 J6M 的 `coverage.yaml`；队列模式没有预先生成整批轨迹，开始时
会统一冻结当时显示的全部参数。
...
| `GOING_TO_START` | 前往起点 | Hybrid A* + TEB 运动学入场，不计覆盖轨迹 |
| `TRANSITING` | 转场中 | 在两条清扫线或两个区域之间点到点导航 |
...
估算。覆盖直线由
`autolabor_coverage/CoverageGlobalPlanner` 强制交给 move_base。普通点到点目标仍委托
Navfn；覆盖任务拥有 move_base 时，所有入场和条带间转场改用 Hybrid A*。管理器先同步调用规划插件的
`/move_base/CoverageGlobalPlanner/set_enforced_path` 服务，在同一个事务中确认任务
所有权、完整运动参数，并切换到“Hybrid A* 转场”或“精确扫掠”模式，之后才提交对应
action goal；后续 topic
只能刷新服务已确认的同一分段，避免跨 transport 到达顺序让旧扫掠路径污染新转场，或让
扫掠短暂走最短路。强制直线路径逐姿态核对完整导航 footprint 和
终点航向。Hybrid A* 在全局代价地图快照上用 `(x,y,yaw,档位,转向档)` 搜索，按 `1.35 m`
最小半径生成恒曲率前进/倒车运动原语，逐小步检查完整车辆 footprint，并把未知格、致命
障碍和地图外视为不可通行。转场可按任务参数允许倒车，覆盖直线始终禁止倒车；覆盖任务的
Hybrid A* 失败或模式交接过期时保持 fail-closed，不会静默退回 Navfn 抄近路。
...
负向速度会被优化器惩罚并在输出端硬截为零。上述直线跟踪权重只用于扫掠；本次前进/
倒车速度、最大角速度和线/角加速度会在每个覆盖分段临时写入 TEB。Hybrid A* 转场保留
全局路径中的车辆朝向和倒车段，使用有序软 via-point 引导 TEB，同时允许 TEB 为实时局部
障碍调整轨迹；任务结束时完整恢复普通导航基线。直线代价不会越过 footprint
碰撞和障碍安全约束；条带被障碍阻断时仍停车、等待和重试。

条带方向不是固定沿地图 X/Y 轴。规划器以 `15°` 间隔角和多边形各条有效边的方向作为
候选，对每个候选按
`车道中心距 = 有效清扫宽度 × (1 - 重叠率)` 切出扫描线，再逐姿态检查完整车辆
footprint、静态障碍和未知区。单区域预览只在定位新鲜时额外按车辆当前位置的自由连通域
裁剪；未定位时可以生成几何预览，但不能据此开始任务。单区域启动重规划和批次逐区即时
规划始终要求当前位姿，并按当前位置可达连通域重新裁剪。覆盖完整度是硬优先级；在可覆盖
面积相同的候选之间，规划器以“秒”为单位比较预计总耗时。所有角度先做快速排序，预计
最好的三个角度再用宽度为 `128` 的确定性 beam search 联合选择第一条线、后续线顺序及
每条线方向，避免只贪心选择最近端点。青色预览只画最终清扫条带，不把条带间的几何连线
冒充成已经验证的可行路线。

兼容接口 `/coverage/planned_path` 仍会把“当前位置到条带、条带到条带”的直线采样拼入
同一条估算 Path，但 Qt/RViz 默认禁用这层，只显示 `/coverage/markers` 中的青色扫掠线。
该 Path 的连接部分不是 Hybrid A* 结果，也绝不能作为可直接执行或已避障的轨迹；实际转场
路线只在执行时由 Hybrid A* + TEB 产生。

“从当前位置去第一条清扫线”和“从上一条清扫线去下一条清扫线”都属于转场，不计入
覆盖轨迹，也不会把两点间的直线强塞给底盘。转场时 Hybrid A* 每秒依据包含实时 `/scan`
障碍的全局代价地图重新搜索满足前轮曲率和档位约束的路线，TEB 以 `10 Hz` 在局部继续优化
并避开新出现的障碍；
在导航链正常且存在足够车辆宽度和转弯半径的通路时，车辆应绕开障碍继续前往条带入口；
障碍封死全部通路时会停车。定位/传感器或安全门失效、规划器 handoff、TEB 配置、
move_base action/service 异常、超时或系统关闭也会停车，并可能进入 `PAUSED`、`CANCELED`
或 `FAILED`。
扫掠段则相反：它要保证不漏扫，因此全局参考线固定为该条带；遇到无法局部避开的障碍会
等待和重试，而不会悄悄改成一条绕开大片待扫区域的最短路。
...
`/m2_driver/chassis_parameter`，并核对 VCU、Hybrid A*、TEB 的轴距、转角、半径和 Twist 输出模式；
不一致时拒绝发车。离线条带排序的秒级代价由以下部分组成：静态图已知自由格上的 A* 转场
距离与无障碍 Dubins 最短曲率路径长度的较大值、按最大
前进/倒车速度和线加速度计算的三角形/梯形加减速时间、按最小转弯半径/最大角速度/角加
速度计算的转向时间、倒车换向附加时间，以及每个 move_base 分段的停车与交接附加时间。
清扫线本身固定前进，转场则比较允许条件下的前进和倒车估算；第一条线和所有后续选择
使用同一个总时间目标。

这个结果是受限搜索得到的“最短预计时间”，不是任意排列的数学全局最优证明。TEB 的原始
图优化 cost 混有障碍、可行性、via-point 等不同量纲的加权残差，不是秒数，因此不会被
直接加进时间目标；规划时使用与执行一致的速度、加速度、曲率和静态图绕行约束。执行阶段
再由带档位的 Hybrid A* 用同一任务速度上限生成可行全局路线，并由在线 TEB 局部优化。
动态行人、实时障碍、局部同伦选择和等待重试会让真实耗时偏离
预估；软件也不会把“路径已生成”表述为整条路线已经通过实体车运动学证明，仍需低速实车
验收。

车辆已经位于条带入口的位置容差内、但车头方向仍超出航向容差时，不会再沿用旧架构直接
判阻塞，也不会要求 Ackermann 底盘原地旋转；Hybrid A* 会尝试驶离入口后用前进/倒车曲线
重新进入。只有周边已知自由空间不足、搜索超时或确实无解时才进入等待和有限重试。

通常每个已提交分段以 move_base action 返回 `SUCCEEDED` 为完成依据，也就是车辆满足该段
终点的位置和航向容差；唯一捷径是转场执行前已经同时满足入口位置和航向容差时，管理器
会直接判定该转场完成而不发送 action goal。系统不会按行驶时间、计划长度或“已经走过
大部分线段”推断完成。实际覆盖面积由扫掠段上的实走位姿按有效宽度栅格化并取并集，重走
和重试不会重复累计。

#### 障碍、暂停和失败恢复

分段 action 返回 `ABORTED/REJECTED/LOST`、执行超时或暂时没有可行路线时，都会按阻塞
处理；动态障碍是最常见原因。任务等待 `10 s`，最多重试 `3` 次后先跳过该段；其余路线
完成后再最终重试 `1` 次，并以“部分完成”和阻塞分段数报告。FOD 进入视觉/安全暂停时，
...
清扫地图的固定颜色含义为：青色是覆盖条带预览，蓝色是 move_base/TEB 接收的全局参考
路线，红色是 TEB 当前局部优化轨迹，绿色是覆盖扫掠段的实际执行记录。入场和换道转场的
蓝/红路线会实时变化，但不累计到绿色覆盖记录；任务终止后这些临时显示会被清空。
