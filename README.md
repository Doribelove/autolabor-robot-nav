...
- NVIDIA 端 22 个包已用 Release 配置编译通过；当前静态检查结果汇总为 763 项通过、
  0 错误、0 失败、0 跳过。
...
- J6M `current` 当前指向版本 `20260828_164358`。时间最短覆盖规划和扩展 ROS 接口已在
  NVIDIA/J6M 成套构建部署，四个共享接口 MD5 一致；远端静态检查与 `latest` 静态地图
  冷启动运行健康检查通过。当前仍在等待操作员按车辆真实位置设置 `/initialpose`，本次
  发布没有发送导航目标或非零速度，尚未进行覆盖运动验收。
...
启动 Qt 时作为默认值；这组偏好不是地图区域记录的一部分，也不会同步到 J6M 参数文件。
队列模式没有预先生成整批轨迹，开始时会统一冻结当时显示的全部参数。
