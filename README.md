...
- NVIDIA 端 22 个包已用 Release 配置编译通过；当前静态检查结果汇总为 769 项通过、
  0 错误、0 失败、0 跳过。
...
4. 检查青色覆盖条带、可覆盖面积、不可覆盖估算和预计总时间。界面不再把条带之间的
   直线连接画成可执行路径；转场的实际蓝/红路线由 Navfn + TEB 在执行时生成。默认有效
...
在不占用状态锁的情况下执行 VCU/TEB 在线核对及连通域重规划，使 `/scan`、双 LD19、定位
...
| `PREPARING` | 正在复核起点与安全门 | 按最新位姿重规划并在线核对底盘、TEB 和感知 |
| `GOING_TO_START` | 前往起点 | Navfn + TEB 点到点入场，不计覆盖轨迹 |
...
终点航向。转场可选让 TEB 按当前“最高倒车速度”尝试倒车，覆盖直线始终禁止倒车。
...
倒车速度、最大角速度和线/角加速度会在每个覆盖分段临时写入 TEB，Navfn 转场仍保留普通
点到点的路径权重，任务结束时完整恢复基线。直线代价不会越过 footprint
...
该 Path 的连接部分不是 Navfn 结果，也绝不能作为可直接执行或已避障的轨迹；实际转场路线
只在执行时由 Navfn + TEB 产生。
...
不一致时拒绝发车。秒级代价由以下部分组成：静态图已知自由格上的 A* 转场距离、按最大
...
直接加进时间目标；规划时只使用与实际 TEB 一致的运动上限和静态图绕行距离。实际转场仍
由 Navfn + 在线 TEB 生成，动态行人、实时障碍、局部同伦选择和等待重试会让真实耗时偏离
